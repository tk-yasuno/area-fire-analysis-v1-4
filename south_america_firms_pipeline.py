#!/usr/bin/env python3
"""
南米地域 NASA FIRMS 森林火災データ分析パイプライン
チリを中心とする南米エリアの火災データを対象とした分析システム
結果は自動生成されるタイムスタンプ付きフォルダに保存

エリア範囲:
- 緯度: 南緯56°～北緯15° (パタゴニア～カリブ海)
- 経度: 西経82°～西経35° (太平洋～大西洋)
- 対象国: チリ、アルゼンチン、ブラジル、ペルー、コロンビア、ボリビア、エクアドル、ベネズエラ等
"""
...
class SouthAmericaFirmsAnalyzer:
    """南米地域NASA FIRMS森林火災分析システム"""
    ...
    def __init__(self, config_path: str = "config_south_america_firms.json"):
        ...
        self.output_dir = f"data_firms_south_america_{timestamp}"
    ...
    def run_pipeline(self) -> str:
        ...
        Returns:
            結果ファイルパス
        """
        logger.info("🌎 Starting South America FIRMS Fire Analysis Pipeline")
        ...
        try:
            ...
            logger.info(f"🎉 South America Fire Analysis Pipeline completed successfully!")
    ...
    def _collect_nasa_firms_data(self):
        """NASA FIRMSデータ収集"""
        logger.info("=== Collecting NASA FIRMS Data (South America Region) ===")
    ...
    def _create_comprehensive_visualizations(self, embeddings, labels, data, scores):
        ...
        try:
            ...
            tsne_plot_path = self.visualization_manager.create_cluster_plot(
                ...
                labels=labels,
                scores=scores_np,
                title="South America Fire Detection Clusters (t-SNE)"
            )
    ...
    def _save_final_results(self, data, labels, clustering_results, feature_analysis):
        ...
        result_path = os.path.join(self.output_dir, "final_south_america_results.json")
        ...
        labeled_data_path = os.path.join(self.output_dir, "south_america_fires_clustered.csv")
    ...
    def _display_results_summary(self, clustering_results, total_time, num_samples):
        """結果サマリー表示"""
        print("\n" + "="*70)
        print("🌎 SOUTH AMERICA FOREST FIRE ANALYSIS RESULTS")
        print("="*70)
        ...
        print(f"🌍 Region Coverage: South America (56°S-15°N, 82°W-35°W)")
        print("="*70)
...
def main():
    ...
    # コマンドライン引数の設定
    parser = argparse.ArgumentParser(description='South America Fire Detection Analysis Pipeline')
    parser.add_argument('--config', default='config_south_america_firms.json',
                       help='Configuration file path (default: config_south_america_firms.json)')
    args = parser.parse_args()
    
    try:
        # 指定された設定ファイルで分析実行
        analyzer = SouthAmericaFirmsAnalyzer(config_path=args.config)
